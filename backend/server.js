...
//route de récupération des usernames
...
// route de redirestion vers la page de profil
...
app.post('/api/publier', verifyToken, multer, async (req, res) => {
    ...
    try {
      ...
      res.status(201).json({ id: results.insertId, message: "Post créé !", imageUrl: imageUrl });
    }catch (error) {
        console.error("Erreur lors de la publication :", error);
        res.status(500).json({ message: "Erreur serveur" });
    }
...
app.get("/api/posts", verifyToken, async (req, res) => {
  try {
    const listPosts = [];
    const [rows] = await db.query("SELECT posts.*, favs.post_id AS isLiked FROM posts LEFT JOIN favs ON posts.id = favs.post_id AND favs.user_id = ? ORDER BY posts.created_at DESC", req.user.id);
    for (const i in rows) {
      try {
...
app.post("/api/addFav/:postId", verifyToken, async (req, res) => {
  ...
  try{
    const [results] = await db.query("INSERT INTO favs (post_id, user_id) VALUES (?, ?)", [postId, userId]);
    const nbLikes = await db.query(
      "SELECT COUNT(*) AS likeCount FROM favs WHERE post_id = ?",
      postId
    );
    res.status(201).json({ id: results.insertId, nbLikes: nbLikes[0][0].likeCount, message: "Ajouté aux favoris"});
  }catch(error){
...
app.delete("/api/removeFav/:postId", verifyToken, async (req, res) => {
  ...
  const userId = req.user.id;
  try {
    const result = await db.query("DELETE FROM favs WHERE post_id = ? AND user_id = ?", [postId, userId]);
    const nbLikes = await db.query(
      "SELECT COUNT(*) AS likeCount FROM favs WHERE post_id = ?",
      postId
    );
    res.status(200).json({message: "Retiré des favoris", nbLikes: nbLikes[0][0].likeCount });
  } catch (error) {
...
app.delete("/api/post/:postId", verifyToken, async (req, res) => {
  ...
  const userId = req.user.id;
  try {
    const [rows] = await db.query("SELECT imageUrl FROM posts WHERE id = ? AND user_id = ?", [postId, userId]);
